...
import { ChannelsListContext } from '../../ChannelsListContext';
...
function DmMessages({ dmId = '' }) {
  ...
  // Added context myself to check for channels list (side panel) 
  // state change when message is sent
  const { myChannelsState, allChannelsState } = React.useContext(ChannelsListContext);
  const [myChannels, setMyChannels] = myChannelsState;
  const [allChannels, setAllChannels] = allChannelsState;
