...
import Face from '@material-ui/icons/Face'; // Icon for creating personal assistant
...
function AddDmDialog({ ...props }) {
  ...
  // Added myself
  function createPersonalAi(event) {

    makeRequest('POST', 'PERSONAL_ASSISTANT_CREATE', { token })
        .then(response => {
          console.log(response);
          props.callback();
        })
  ...
  return (
      <div>
        { /* Added the below button myself */}
        <IconButton size="small" onClick={createPersonalAi}>
          <Face/>
        </IconButton>
